...
import { Check, ClipboardCheck, Plus, ShieldCheck, Trash2 } from 'lucide-react';
import {
  getActiveChurchId,
...
const legacyKey = 'digital-church-requests-desk';
const localPrefix = 'digital-church-requests-desk:v2';
...
function normalizeItems(value: unknown): Item[] {
  ...
  return value.filter((item) => item && typeof item === 'object').map((item: any, index) => ({
    id: typeof item.id === 'string' ? item.id : `request-${index}`,
    type: ['baptism', 'membership', 'volunteer', 'event', 'facility', 'letter', 'care', 'benevolence', 'other'].includes(item.type) ? item.type : 'other',
    requester: typeof item.requester === 'string' ? item.requester : 'New request',
    owner: typeof item.owner === 'string' ? item.owner : '',
    status: ['submitted', 'assigned', 'waiting', 'completed', 'declined'].includes(item.status) ? item.status : 'submitted',
...
export function ChurchRequestsDesk() {
  ...
  const loadWorkspace = async (churchId: string) => {
    ...
    setSaved(false);
    setSyncing(true);
    setSyncMessage(churchId ? 'Loading church request desk…' : 'Loading private request draft…');
    try {
  ...
  const save = async () => {
    setSyncing(true);
    setSyncMessage(activeChurchId ? 'Saving requests to active church…' : 'Saving private request draft…');
    try {
      const result = await saveChurchOperationalRecord({
        churchId: activeChurchId,
        module: 'requests',
        recordKey: 'desk',
        title: 'Church requests & forms desk',
