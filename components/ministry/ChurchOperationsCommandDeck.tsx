...
type Area = { id: string; title: string; description: string; href: string; icon: typeof Church; critical?: boolean };
type WeeklyOpsState = { ready: string[]; priority: string; risk: string; win: string };
...
const areas: Area[] = [
  ...
  { id: 'care', title: 'Prayer & pastoral care', description: 'Prayer requests, human care, appointments, referrals, safeguarding, and accountable follow-up.', href: '/care/manage', icon: HeartHandshake, critical: true },
  { id: 'follow-up', title: 'Discipleship follow-up', description: 'Consent-aware response ownership across contact, foundations, baptism, belonging, groups, and serving.', href: '/follow-up/manage', icon: UsersRound, critical: true },
  { id: 'attendance', title: 'Attendance & assimilation', description: 'Aggregate attendance, guest flow, response ownership, and connection signals without individual surveillance.', href: '/attendance/manage', icon: UsersRound },
  { id: 'groups', title: 'Small groups & community', description: 'Group leadership depth, meeting rhythm, healthy capacity, and places for people to belong.', href: '/groups/manage', icon: Church },
...
function normalizeWeeklyState(value: unknown): WeeklyOpsState {
  const data = value && typeof value === 'object' ? value as any : {};
  return {
    ready: Array.isArray(data.ready) ? data.ready.filter((item: unknown): item is string => typeof item === 'string') : [],
    priority: typeof data.priority === 'string' ? data.priority : '',
    risk: typeof data.risk === 'string' ? data.risk : '',
    win: typeof data.win === 'string' ? data.win : '',
  };
}

const emptyState: WeeklyOpsState = { ready: [], priority: '', risk: '', win: '' };
...
export function ChurchOperationsCommandDeck() {
  ...
  const applyState = (data: WeeklyOpsState) => { setReady(data.ready); setPriority(data.priority); setRisk(data.risk); setWin(data.win); };

  const loadWorkspace = async (churchId: string) => {
    const week = currentWeekStart();
    setActiveChurchId(churchId); setSaved(false); setSyncing(true);
    setSyncMessage(churchId ? 'Loading shared church week…' : 'Loading private weekly draft…');
    try {
      const result = await loadChurchOperationalRecord({
        churchId,
        module: 'command-center',
        recordKey: `week:${week}`,
        localStoragePrefix: `digital-church-ops-week:v2:${week}`,
        legacyLocalStorageKey: `digital-church-ops-week:${week}`,
        legacyScopedLocalStorageKey: churchId ? `digital-church-ops-week:${churchId}:${week}` : undefined,
        defaultValue: emptyState,
        normalize: normalizeWeeklyState,
      });
  ...
  const save = async () => {
    const week = currentWeekStart();
    setSyncing(true); setSyncMessage(activeChurchId ? 'Saving to active church…' : 'Saving private weekly attention…');
    try {
      const result = await saveChurchOperationalRecord({
        churchId: activeChurchId,
        module: 'command-center',
        recordKey: `week:${week}`,
        title: `Weekly church operations · ${week}`,
        localStoragePrefix: `digital-church-ops-week:v2:${week}`,
        value: { ready, priority, risk, win },
      });
    ...
    <section className="overflow-hidden rounded-[2rem] border border-stone-200 bg-white shadow-sm">
      <div className="grid xl:grid-cols-[1.18fr_0.82fr]">
        <div className="p-6 sm:p-8 lg:p-10">
          <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between"><div><div className="inline-flex items-center rounded-full bg-sage-50 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.18em] text-sage-700"><Sparkles className="mr-2 h-4 w-4" /> Church operations command deck</div><h2 className="mt-4 max-w-4xl text-3xl font-light leading-tight text-stone-900 md:text-4xl">One weekly view for the ministries people actually depend on.</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-stone-600">Use readiness as an operational checklist—not a ministry score. With an active church workspace, weekly state is versioned and shared only with authorized leaders for that church.</p><div className="mt-3 inline-flex items-center gap-2 rounded-full border border-stone-200 bg-stone-50 px-3 py-1.5 text-xs font-semibold text-stone-600"><ShieldCheck className="h-3.5 w-3.5 text-sage-700" /> {syncing ? 'Syncing…' : syncMessage}</div></div><div className="min-w-[180px] rounded-2xl bg-stone-950 p-4 text-white"><p className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Weekly readiness</p><p className="mt-1 text-4xl font-light">{readiness}%</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full bg-sage-400" style={{ width: `${readiness}%` }} /></div><p className={`mt-3 text-xs ${criticalReady ? 'text-sage-300' : 'text-amber-300'}`}>{criticalReady ? 'Critical areas reviewed' : 'Critical review still open'}</p></div></div>

          <div className="mt-7 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{areas.map((area) => { const Icon = area.icon; const done = ready.includes(area.id); return <article key={area.id} className={`rounded-2xl border p-4 transition ${done ? 'border-sage-200 bg-sage-50' : area.critical ? 'border-amber-200 bg-amber-50/50' : 'border-stone-200 bg-stone-50'}`}><div className="flex items-start justify-between gap-3"><span className={`flex h-10 w-10 items-center justify-center rounded-xl ${done ? 'bg-sage-600 text-white' : 'bg-white text-stone-700 shadow-sm'}`}>{done ? <Check className="h-5 w-5" /> : <Icon className="h-5 w-5" />}</span>{area.critical && <span className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-amber-800">Critical</span>}</div><h3 className="mt-4 font-semibold text-stone-900">{area.title}</h3><p className="mt-1 min-h-[60px] text-xs leading-5 text-stone-500">{area.description}</p><div className="mt-4 flex items-center justify-between gap-2"><Link href={area.href} className="text-xs font-semibold text-sage-700">Open module →</Link><button type="button" onClick={() => toggle(area.id)} className={`rounded-full px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider ${done ? 'bg-white text-sage-700' : 'bg-stone-200 text-stone-600'}`}>{done ? 'Reviewed' : 'Mark reviewed'}</button></div></article>; })}</div>
        </div>

        <aside className="bg-stone-950 p-6 text-white sm:p-8 lg:p-10"><ShieldCheck className="h-8 w-8 text-sage-300" /><h3 className="mt-5 text-3xl font-light">Leadership attention, not surveillance.</h3><p className="mt-3 text-sm leading-6 text-stone-400">Record the week’s biggest priority, risk, and win. Keep sensitive pastoral details inside the appropriate care systems rather than this general leadership scratchpad.</p><label className="mt-7 block"><span className="mb-2 block text-xs font-bold uppercase tracking-wider text-stone-400">This week’s priority</span><textarea value={priority} onChange={(e) => setPriority(e.target.value)} className="min-h-[100px] w-full rounded-2xl border border-white/10 bg-white/5 p-4 text-sm leading-6 text-white outline-none focus:ring-2 focus:ring-sage-400" placeholder="What must leadership make sure happens well?" /></label><label className="mt-5 block"><span className="mb-2 block text-xs font-bold uppercase tracking-wider text-stone-400">Risk / gap</span><textarea value={risk} onChange={(e) => setRisk(e.target.value)} className="min-h-[100px] w-full rounded-2xl border border-white/10 bg-white/5 p-4 text-sm leading-6 text-white outline-none focus:ring-2 focus:ring-amber-400" placeholder="Coverage gap, unresolved care, rights issue, technical dependency..." /></label><label className="mt-5 block"><span className="mb-2 block text-xs font-bold uppercase tracking-wider text-stone-400">Win / testimony to review</span><textarea value={win} onChange={(e) => setWin(e.target.value)} className="min-h-[100px] w-full rounded-2xl border border-white/10 bg-white/5 p-4 text-sm leading-6 text-white outline-none focus:ring-2 focus:ring-sage-400" placeholder="What went well and should be thanked, learned from, or followed up?" /></label><button type="button" onClick={() => void save()} disabled={syncing} className="mt-5 inline-flex w-full items-center justify-center rounded-xl bg-sage-500 px-5 py-3 text-sm font-semibold text-white hover:bg-sage-400 disabled:cursor-not-allowed disabled:opacity-60">{saved ? <Check className="mr-2 h-4 w-4" /> : <ClipboardCheck className="mr-2 h-4 w-4" />}{syncing ? 'Syncing weekly attention…' : saved ? 'Weekly attention saved' : activeChurchId ? 'Save to active church' : 'Save private weekly attention'}</button><p className="mt-4 text-xs leading-5 text-stone-500">This operational record must not contain confidential counseling notes, medical details, abuse reports, financial credentials, child safeguarding case content, or other restricted case data.</p></aside>
      </div>
    </section>
  );
}
