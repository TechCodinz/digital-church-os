...
export const Navbar = () => {
  ...
  const navItems = [
    ...
    { name: 'Live Service', href: '/live-service' },
    { name: 'Prayer Room', href: '/prayer-room' },
  ...
  return (
    <nav className="glass-morphism fixed w-full z-50">
          ...
          <div className="hidden xl:flex items-center space-x-6">
            ...
            {session ? (
              <div className="relative" ref={dropdownRef}>
                ...
                <AnimatePresence>
                  {dropdownOpen && (
                    <motion.div initial={{ opacity: 0, y: 8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.96 }} transition={{ duration: 0.15 }} className="absolute right-0 top-14 w-56 bg-white rounded-2xl shadow-xl border border-stone-100 py-2 overflow-hidden">
      ...
      <motion.div initial={false} animate={isOpen ? { height: 'auto', opacity: 1 } : { height: 0, opacity: 0 }} className="xl:hidden overflow-hidden bg-white/95">
        <div className="px-4 py-2 space-y-0">
          {navItems.map((item) => (
            <Link key={item.name} href={item.href} className="block py-3 text-stone-600 hover:text-sage-600 border-b border-cream-200" onClick={() => setIsOpen(false)}>
              {item.name}
            </Link>
          ))}
          {session ? (
