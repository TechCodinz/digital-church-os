      ...
      <section className="sanctuary-cinematic-hero relative overflow-hidden px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1fr_0.68fr] lg:items-end">
          <div className="relative z-10 max-w-4xl">
            <div className="inline-flex items-center rounded-full border border-amber-200/20 bg-white/5 px-4 py-2 text-sm font-medium text-amber-100 backdrop-blur-xl"><Sparkles className="mr-2 h-4 w-4" /> Fasting & prayer sanctuary</div>
        ...
        <div className="relative mx-auto max-w-7xl">
            ...
            <div className="sacred-panel-dark p-6 sm:p-8">
              ...
              <div className="mt-7 grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  return (
                    <button key={item.id} type="button" onClick={() => { markChanged(); setPractice(item.id as typeof practice); }} className={`rounded-2xl border p-4 text-left transition ${active ? 'border-amber-200/30 bg-amber-300/10 shadow-[0_16px_42px_rgba(245,201,120,.08)]' : 'border-white/10 bg-white/[0.035] hover:border-white/18 hover:bg-white/[0.055]'}`}>
              ...
              <label className="mt-7 block"><span className="mb-2 block text-xs font-bold uppercase tracking-wider text-white/35">Purpose</span><textarea value={purpose} onChange={(e) => { setPurpose(e.target.value); markChanged(); }} maxLength={1600} className="min-h-[120px] w-full rounded-2xl border border-white/10 bg-black/20 p-4 leading-6 text-white outline-none placeholder:text-white/20 focus:ring-2 focus:ring-amber-200/45" placeholder="Why are you setting apart this time? What do you want to seek God about?" /></label>

              <div className="mt-6">
                <p className="mb-3 text-xs font-bold uppercase tracking-wider text-white/35">Prayer focuses</p>
                <div className="flex flex-wrap gap-2">{focusOptions.map((focus) => <button key={focus} type="button" onClick={() => toggleFocus(focus)} className={`rounded-full border px-4 py-2 text-sm font-medium transition ${focuses.includes(focus) ? 'border-emerald-200/30 bg-emerald-300/12 text-emerald-100' : 'border-white/10 bg-white/5 text-white/50 hover:border-white/18'}`}>{focuses.includes(focus) && <Check className="mr-1 inline h-3.5 w-3.5" />}{focus}</button>)}</div>
              </div>

              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <label><span className="mb-2 block text-xs font-bold uppercase tracking-wider text-white/35">Scripture references</span><textarea value={scriptureRefs} onChange={(e) => { setScriptureRefs(e.target.value); markChanged(); }} maxLength={1200} className="min-h-[120px] w-full rounded-2xl border border-white/10 bg-black/20 p-4 text-white outline-none placeholder:text-white/20 focus:ring-2 focus:ring-amber-200/45" placeholder="e.g. Isaiah 58, Matthew 6:16-18" /></label>
                <label><span className="mb-2 block text-xs font-bold uppercase tracking-wider text-white/35">Private reflection</span><textarea value={notes} onChange={(e) => { setNotes(e.target.value); markChanged(); }} maxLength={2200} className="min-h-[120px] w-full rounded-2xl border border-white/10 bg-black/20 p-4 text-white outline-none placeholder:text-white/20 focus:ring-2 focus:ring-amber-200/45" placeholder="Prayer burdens, gratitude, insights, answered prayer..." /></label>
              </div>

              <div className="mt-6 flex flex-wrap items-end gap-3">
                <label><span className="mb-2 block text-xs font-bold uppercase tracking-wider text-white/35">Plan length</span><select value={days} onChange={(e) => { markChanged(); setDays(Number(e.target.value)); setCompletedDays([]); }} className="rounded-xl border border-white/10 bg-[#071512] px-4 py-3 text-white outline-none"><option value={1}>1 day</option><option value={3}>3 days</option><option value={7}>7 days</option><option value={14}>14 days</option><option value={21}>21 days</option></select></label>
                <button type="button" onClick={savePlan} disabled={saving} className="inline-flex items-center rounded-full bg-gradient-to-r from-amber-200 to-amber-100 px-5 py-3 text-sm font-semibold text-[#07110f] shadow-[0_14px_36px_rgba(245,201,120,.14)] disabled:cursor-not-allowed disabled:opacity-60">{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <NotebookPen className="mr-2 h-4 w-4" />}{saving ? 'Saving privately…' : 'Save private plan'}</button>
          ...
          <div className="mt-8 sacred-panel-dark p-6 sm:p-8">
            <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
              <div><p className="sanctuary-section-label text-emerald-200/55">Daily rhythm</p><h2 className="mt-2 text-3xl font-light text-white">Pray. Read. Reflect. Serve. Rest.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-white/45">Marking a day simply tracks your private plan. It is not a measure of holiness, sacrifice, favor, or spiritual maturity.</p></div>
              <div className="min-w-[180px] rounded-2xl border border-white/10 bg-white/5 p-4"><div className="flex justify-between text-xs font-semibold text-white/55"><span>Plan completion</span><span>{progress}%</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/8"><div className="h-full rounded-full bg-gradient-to-r from-emerald-300 to-amber-200 transition-all" style={{ width: `${progress}%` }} /></div></div></div>
            </div>
            <div className="mt-6 grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-7">{Array.from({ length: days }, (_, index) => index + 1).map((day) => { const done = completedDays.includes(day); return <button key={day} type="button" onClick={() => toggleDay(day)} className={`rounded-2xl border p-4 text-left transition ${done ? 'border-emerald-200/25 bg-emerald-300/10' : 'border-white/10 bg-white/[0.035]'}`}><CalendarDays className={`h-5 w-5 ${done ? 'text-emerald-100' : 'text-white/30'}`} /><p className="mt-3 font-semibold text-white">Day {day}</p><p className="mt-1 text-xs text-white/35">{done ? 'Reflection recorded' : 'Open for prayer'}</p></button>; })}</div>
          </div>
        </div>
      </section>
    </main>
