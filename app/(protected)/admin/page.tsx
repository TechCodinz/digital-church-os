...
export default function AdminDashboard() {
    ...
    // Fallback to safely guard against missing session or role during redirect
    ...
    return (
        <div className="min-h-screen pt-20 bg-cream-50">
            <div className="max-w-7xl mx-auto px-4 py-8">
                <h1 className="text-3xl font-light text-stone-800 mb-8">Admin Dashboard</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="sanctuary-card">
                        ...
                        <div className="space-y-3">
                            <Link href="/admin/events" className="block w-full p-3 text-left bg-cream-100 rounded-xl hover:bg-cream-200 transition-colors">
                                Create New Event
                            </Link>
                            <Link href="/admin/posts" className="block w-full p-3 text-left bg-cream-100 rounded-xl hover:bg-cream-200 transition-colors">
                                Review Pending Posts
                            </Link>
                            <Link href="/admin/aid-requests" className="block w-full p-3 text-left bg-cream-100 rounded-xl hover:bg-cream-200 transition-colors">
                                Allocate Funds
                            </Link>
                            <Link href="/admin/analytics" className="block w-full p-3 text-left bg-cream-100 rounded-xl hover:bg-cream-200 transition-colors">
                                View Analytics
                            </Link>
                        </div>
                    </div>
                            ...
                            {recentActivity.length > 0 ? recentActivity.map((item, i) => (
                                <div key={i} className="flex items-start space-x-3">
                                    <div className="w-5 h-5 mt-1 text-sage-500">
                                        {item.icon === 'user' ? <Users className="w-5 h-5 text-blue-500" /> :
                                            item.icon === 'post' ? <MessageSquare className="w-5 h-5 text-amber-500" /> :
