...
function safeDesignation(value: unknown) {
...
async function recordOffering(params: {
  ...
  const designation = safeDesignation(params.designation);
...
async function handleSuccessfulPayment(paymentIntent: Stripe.PaymentIntent) {
  ...
  if (created) await runPostPaymentEffects(offering, userId, purpose, true);
}
...
async function handleRecurringPayment(stripe: Stripe, invoice: Stripe.Invoice) {
  ...
  if (created) await runPostPaymentEffects(offering, userId, purpose, false);
}
